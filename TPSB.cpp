...
{
	if (argc < 2) {
		cout << "erro: Voce deve passar o nome do arquivo de entrada e o de saida!" << endl;
		return 0;
	}
	...
	int pep = 1;		//registrador de estado do processador: 0 = positivo, 1 = zero, 2 = negativo
	...
	if (entrada.is_open())
	{
		...
		while (cod_inst != 0)
		{
			...
			//modo verbose
			if (verbose != 0) {
				cout << cod_inst << " " << PC << " " << ap << " " << pep << " " << registerFile[0] << " " << registerFile[1] << " " << registerFile[2] << " " << registerFile[3] << endl;
			}
