...
{
	if (argc < 3) {
		cout << "Erro: argumentos invalidos! Passe o nome do arquivo de entrada e o de saida!" << endl;
		return 0;
	}
	...
	label tabelaLabel[100];			//tabela de labels (assumindo que não irá passar de 100)
	...
	while (!entrada.eof())
	{
		...
		if (linha.length())
		{
			...
			if (linha[0] != ';')
			{
				...
				else if ((op == "JUMP") || (op == "JZ") || (op == "JNZ") || (op == "JN") || (op == "JNN") || (op == "CALL"))
				{
					if (op == "JUMP") {
						programa << 8 << " ";
					}
